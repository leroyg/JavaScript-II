...
let allCaps = [runners.map((name, index, data) => {
    return {
        'name': name.first_name.toUpperCase()
    };
})];
console.log(allCaps);
...
// const filterLargeShirts = data.filter((state) => {
...
runners.forEach(coNames => {
    ...
    companyName.push(coNames.company_name);
});
companyName.sort();
...
// .unique companyName array [];
let uniqueCompanyName = [...new Set(companyName)]; /// ES6 syntax for unique - pretty awesome
console.table(uniqueCompanyName);
let tempObject = {};
let newArray = [];
uniqueCompanyName.forEach(allTheNames => {
    tempObject = companyName.filter(specialNames => {
        return specialNames === allTheNames
    });
    newArray.push({
        companyName:`${allTheNames}`, runnerQuantity: `${tempObject.length}`
    });
})

let vipCo = newArray.filter((moreThanOne) => {
    return moreThanOne.runnerQuantity > 1;
});
...
// });
// forEach on newArray, 
// .filter on tempObject for companyName;
//  push newArray [Object] companyName.length, tempObject.length (quantityRunners)
// filter newArray to only return where qtyRunners > 1 
// forEach companyName 

// Problem 3
// Find all donations over $150 AND add the big money donors to figure out their total! 
let bigMoneyDonors = [];
bigMoneyDonors = runners.filter((large) => {
    return large.donation > 150;
}).reduce((regMoney, bigMoney) => {
    return regMoney += bigMoney.donation;
}, 0);

console.table(bigMoneyDonors);
